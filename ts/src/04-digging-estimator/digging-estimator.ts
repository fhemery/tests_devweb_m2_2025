...
export interface DiggingRateApiInterface{
  get(rockType: string): number[];
}

class VinDiggingRateApi implements DiggingRateApiInterface {
  get(rockType: string): number[] {
...
export class DiggingEstimator {
  constructor(private diggingRateApi: DiggingRateApiInterface
     = new VinDiggingRateApi()
  ) {}

  tunnel(length: number, days: number, rockType: string): TeamComposition {
    const digPerRotation = this.diggingRateApi.get(rockType);
